...
- Do not embed a map.
- Images are decorative generated assets in `assets/` with empty `alt`: PNGs for fast-view/guide illustrations and product-focused card photos, and SVGs for genre icons, storefronts, and product illustrations. Do not add photographic hero images or fake real-store photos; photo-style sample assets should show products/menus, avoid logos/readable text/people, and be referenced through `cardImage` fields in `stores.json`. Per-store and per-product illustration SVGs are generated by `tools/generate_*.py` and referenced via `image` fields in `stores.json`.
